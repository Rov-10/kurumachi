...
export default function WiringTab() {
  const pinout = [
    ...
    {
      mcu: 'GPIO 3',
      device: 'TP4057 Battery Node (via 100k+100k Divider)',
      type: 'Analog Input (ADC VBAT)',
    },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <tbody className="divide-y divide-nothing-border/30 text-nothing-text/80">
                {pinout.map((pin, idx) => (
                  <tr key={idx} className="hover:bg-white/5 transition-colors">
                    <td className="py-3.5 font-mono text-xs text-nothing-red whitespace-nowrap">
                      {pin.mcu}
                    </td>
        ...
        <div className="lg:col-span-1 border border-nothing-border bg-black/40 rounded-3xl p-6 flex flex-col justify-between">
          <div className="space-y-4">
            <h4 className="font-dot text-sm text-white uppercase tracking-wider">
              Engineering Notes
            </h4>
            <ul className="space-y-3 font-space text-xs text-nothing-text/60 list-disc list-inside leading-relaxed">
              <li>
                <strong>I2C Bus Topography:</strong> All peripheral modules (OLED, IMU, Environment
                Matrix) are wired in parallel to <span className="text-white">GPIO 8 (SDA)</span>{' '}
                and <span className="text-white">GPIO 9 (SCL)</span>.
              </li>
              <li>
                <strong>ADC Protection Circuit:</strong> A{' '}
                <span className="text-nothing-red">100kΩ + 100kΩ voltage divider</span> is mandatory
                between the Li-Po battery and GPIO 3 (VBAT). It halves the 4.2V peak voltage to a
                safe 2.1V for the ESP32-C3 ADC logic.
              </li>
              <li>
                <strong>Power Routing:</strong> Main power runs from the battery → Slide Switch →
                1N4007 Diode → ESP32 5V Pin, utilizing the onboard LDO to supply stable 3.3V to the
                sensors.
              </li>
            </ul>
          </div>
