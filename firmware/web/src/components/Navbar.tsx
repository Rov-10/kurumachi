  ...
  return (
    <div className="w-full flex justify-center fixed top-0 z-50 pointer-events-none">
      <nav className="w-full max-w-7xl flex justify-between items-center p-6 border-b border-nothing-border backdrop-blur-md bg-black/10 pointer-events-auto">
        ...
        {/* Центральні лінки (приховані на мобілках) */}
        <div className="hidden md:flex items-center gap-8 font-dot text-xs tracking-widest uppercase">
          {links.map((link) => (
            <Link
              key={link.path}
              href={link.path}
        ...
        {/* Оригінальна кнопка (тепер веде на Web Installer) */}
        <Link 
          href="/installer"
